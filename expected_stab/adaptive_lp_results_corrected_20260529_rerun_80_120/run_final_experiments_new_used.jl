...
# LP solvers are loaded at include time so JuMP sees a normal optimizer
# constructor inside compiled experiment functions.
const _LP_OPTIMIZER_ERROR = Ref("")
const _LP_OPTIMIZER = let
    try
        ...
        try
            ...
            io1 = IOBuffer()
            io2 = IOBuffer()
            showerror(io1, highs_err)
            showerror(io2, glpk_err)
            _LP_OPTIMIZER_ERROR[] = "HiGHS: $(String(take!(io1))) | GLPK: $(String(take!(io2)))"
            nothing
        end
    end
end

_lp_optimizer() = _LP_OPTIMIZER

function require_lp_optimizer()
    optimizer = _lp_optimizer()
    if optimizer === nothing
        error("No LP optimizer found. Tried HiGHS and GLPK. Errors: " * _LP_OPTIMIZER_ERROR[])
    end
    return optimizer
end

# Optional SDP solvers are also lazy so non-SDP runs do not pay solver
# precompile/startup costs.
const _SDP_OPTIMIZER_CACHE = Ref{Any}(nothing)

function _sdp_optimizer()
    cached = _SDP_OPTIMIZER_CACHE[]
    cached !== nothing && return cached
    for pkg in (:MosekTools, :SCS, :COSMO)
        try
            if pkg == :MosekTools
                Base.eval(@__MODULE__, :(using MosekTools))
                optimizer = getfield(getfield(@__MODULE__, :MosekTools), :Optimizer)
            elseif pkg == :SCS
                Base.eval(@__MODULE__, :(using SCS))
                optimizer = getfield(getfield(@__MODULE__, :SCS), :Optimizer)
            else
                Base.eval(@__MODULE__, :(using COSMO))
                optimizer = getfield(getfield(@__MODULE__, :COSMO), :Optimizer)
            end
            _SDP_OPTIMIZER_CACHE[] = optimizer
            return optimizer
        catch
        end
    end
...
function generate_scenario_pool(job::JobDistribution, k::Int, num_samples::Int; rng=Random.GLOBAL_RNG)
    pool = Matrix{Int}(undef, num_samples, 2)
    for r in 1:num_samples
        s = round(Int, clamp(rand(rng, job.start_dist), 1, k))
        e = round(Int, clamp(rand(rng, job.end_dist), 1, k))
        while e < s
            e = round(Int, clamp(rand(rng, job.end_dist), 1, k))
        end
        pool[r, 1] = s
...
function generate_scenario_pools(jobs::Vector{JobDistribution}, k::Int, num_samples::Int; rng=Random.GLOBAL_RNG)
    return [generate_scenario_pool(job, k, num_samples; rng=rng) for job in jobs]
end

function scenario_position_probabilities(pools::Vector{Matrix{Int}}, k::Int)
    n = length(pools)
    P_s = zeros(n, k)
    P_e = zeros(n, k)
    P_occ = zeros(n, k)
    ...
    for i in 1:n
        pool = pools[i]
        sample_count = size(pool, 1)
        sample_count == 0 && continue
        for r in 1:sample_count
            s = pool[r, 1]
            e = pool[r, 2]
            P_s[i, s] += 1
            P_e[i, e] += 1
        ...
        P_s[i, :] ./= sample_count
        P_e[i, :] ./= sample_count
        P_occ[i, :] ./= sample_count
        L[i] /= sample_count
    end
    return P_s, P_e, P_occ, L
end

function scenario_support_bounds(pools::Vector{Matrix{Int}}, k::Int)
    n = length(pools)
    a = ones(Int, n)
    b = ones(Int, n)
    c = ones(Int, n)
    ...
    for i in 1:n
        ...
        a[i] = max(1, minimum(pool[:, 1]))
        b[i] = min(k, maximum(pool[:, 1]))
        c[i] = max(1, minimum(pool[:, 2]))
        d[i] = min(k, maximum(pool[:, 2]))
    end
    return a, b, c, d
end

function interval_overlaps_pool(pool::Matrix{Int}, s::Int, e::Int)
    overlap = falses(size(pool, 1))
    for r in 1:size(pool, 1)
        overlap[r] = max(s, pool[r, 1]) <= min(e, pool[r, 2])
    end
    return overlap
end
...
lp_is_solved(status) = status == MOI.OPTIMAL || status == MOI.LOCALLY_SOLVED

function lp_solution_conservative(P_occ::AbstractMatrix, weights::AbstractVector)
    optimizer = _lp_optimizer()
    optimizer === nothing && return NaN, fill(NaN, length(weights)), "NO_OPTIMIZER"
    ...
    if !lp_is_solved(status)
        return NaN, fill(NaN, n), string(status)
    end
    return objective_value(model), value.(x), string(status)
end
...
function lp_bound_conservative(P_occ::AbstractMatrix, weights::AbstractVector)
    obj, _, _ = lp_solution_conservative(P_occ, weights)
    return obj
end

function lp_solution_original_from_bounds(P_s::AbstractMatrix, P_e::AbstractMatrix,
    weights::AbstractVector, m::Int, a::Vector{Int}, b::Vector{Int}, c::Vector{Int}, d::Vector{Int})
    ...
    return objective_value(model), value.(x[:, 1]), string(status)
end
...
function lp_bound_original(jobs::Vector{JobDistribution}, P_s::AbstractMatrix, P_e::AbstractMatrix, weights::AbstractVector, m::Int)
    ...
    obj, _, _ = lp_solution_original_from_bounds(P_s, P_e, weights, m, a, b, c, d)
    return obj
end
...
function add_gap_columns!(df::DataFrame)
    ...
    if "cdsrse_lp_adaptive_over_opt" in names(df)
        df.cdsrse_lp_adaptive_gap = abs.(1 .- df.cdsrse_lp_adaptive_over_opt)
    end
    if "dsrse_lp_adaptive_over_opt" in names(df)
        df.dsrse_lp_adaptive_gap = abs.(1 .- df.dsrse_lp_adaptive_over_opt)
    end
    return df
end
...
function select_largest_score(scores::AbstractVector, weights::AbstractVector)
    cleaned = collect(scores)
    for i in eachindex(cleaned)
        if !isfinite(cleaned[i])
            cleaned[i] = -Inf
        end
    end
    if all(x -> x == -Inf, cleaned)
        return argmax(weights)
    end
    return argmax(cleaned)
end

function select_largest_lp_score(scores::AbstractVector)
...
function finite_mean(values::AbstractVector)
    finite_values = collect(filter(isfinite, values))
    return isempty(finite_values) ? NaN : mean(finite_values)
end

function simulate_policy_original_lp_adaptive(jobs::Vector{JobDistribution}, weights::AbstractVector, k::Int;
    num_samples::Int=500, num_scenario_samples::Int=500, rng=Random.GLOBAL_RNG)
    ...
    for sim in 1:num_samples
        pools = generate_scenario_pools(jobs, k, num_scenario_samples; rng=rng)
        remaining = collect(1:n)
        total = 0.0
        failed_simulation = false
        while !isempty(remaining)
            active_pools = [pools[i] for i in remaining]
            P_s, P_e, _, _ = scenario_position_probabilities(active_pools, k)
            a, b, c, d = scenario_support_bounds(active_pools, k)
            _, x_first, _ = lp_solution_original_from_bounds(P_s, P_e, weights[remaining], k, a, b, c, d)
            total_solves += 1
            pick_idx = select_largest_lp_score(x_first)
            if pick_idx === nothing
                failed_solves += 1
            ...
            selected_pool = pools[i_sel]
            draw = rand(rng, 1:size(selected_pool, 1))
            s_i = selected_pool[draw, 1]
            e_i = selected_pool[draw, 2]
            total += weights[i_sel]

            new_remaining = Int[]
            for j in remaining
                j == i_sel && continue
                overlap = interval_overlaps_pool(pools[j], s_i, e_i)
                p_conflict = count(overlap) / length(overlap)
                if rand(rng) < p_conflict
                    continue
                end
                keep_rows = findall(.!overlap)
                if !isempty(keep_rows)
                    pools[j] = pools[j][keep_rows, :]
                    push!(new_remaining, j)
                end
            end
            remaining = new_remaining
        end
        all_weights[sim] = failed_simulation ? NaN : total
    end
    return finite_mean(all_weights), all_weights, failed_solves, total_solves
end

function simulate_policy_conservative_lp_adaptive(jobs::Vector{JobDistribution}, weights::AbstractVector, k::Int;
    num_samples::Int=500, P_occ::Union{Nothing,AbstractMatrix}=nothing, rng=Random.GLOBAL_RNG)

    n = length(jobs)
    A_L, A_R = compute_A_intervals(jobs, k)
    P_occ === nothing && (P_occ = estimate_position_probabilities(jobs, k; num_samples=1000)[3])
    all_weights = zeros(Float64, num_samples)
    failed_solves = 0
    total_solves = 0
    for sim in 1:num_samples
        ...
        failed_simulation = false
        while !isempty(remaining)
            _, x_score, _ = lp_solution_conservative(P_occ[remaining, :], weights[remaining])
            total_solves += 1
            pick_idx = select_largest_lp_score(x_score)
            if pick_idx === nothing
                failed_solves += 1
                failed_simulation = true
                break
            ...
            i_sel = remaining[pick_idx]
            s_i, e_i = sample_job_intervals([jobs[i_sel]], k; rng=rng)
            s_i = Int(s_i[1])
            e_i = Int(e_i[1])
            total += weights[i_sel]
...
function run_final_experiments_design(; rng_seed=38072,
    ...
    sdp_n_max = 12,
    run_sdp = false,
    run_adaptive_lp = true,
    num_instances=10,
    num_samples_bounds=1000,
    num_samples_overlap=500,
    num_samples_policy=1000,
    num_samples_adaptive_lp=100,
    num_scenario_samples=500,
    num_samples_opt=1000,
    output_dir::String=".",
    checkpoint_raw_path::Union{Nothing,String}="final_experiment_new_raw_checkpoint.csv")

    mkpath(output_dir)
    output_path(path::String) = isabspath(path) ? path : joinpath(output_dir, path)
    require_lp_optimizer()
        ...
        for len in lengths
            ...
            for dens in densities
                ...
                for dt in dist_types
                    ...
                    n_active = length(weights_use)
                    a_lp = Vector{Int}(undef, n_active)
                    b_lp = Vector{Int}(undef, n_active)
                    c_lp = Vector{Int}(undef, n_active)
                    d_lp = Vector{Int}(undef, n_active)
                    for i in 1:n_active
                        a_lp[i], b_lp[i] = interval_bounds(jobs[i].start_dist, m_used)
                        c_lp[i], d_lp[i] = interval_bounds(jobs[i].end_dist, m_used)
                    end
                    lp_orig, _, lp_orig_status =
                        lp_solution_original_from_bounds(P_s, P_e, weights_use, m_used,
                            a_lp, b_lp, c_lp, d_lp)
                    sdp_orig = (run_sdp && length(weights_use) <= sdp_n_max) ?
                        sdp_bound_original(jobs, P_s, P_e, weights_use, m_used) : NaN
                    cons_lp_adaptive = NaN
                    cons_lp_adaptive_failed_solves = 0
                    cons_lp_adaptive_total_solves = 0
                    orig_lp_adaptive = NaN
                    orig_lp_adaptive_failed_solves = 0
                    orig_lp_adaptive_total_solves = 0
                    if run_adaptive_lp
                        cons_lp_adaptive, _, cons_lp_adaptive_failed_solves,
                            cons_lp_adaptive_total_solves =
                            simulate_policy_conservative_lp_adaptive(jobs, weights_use, m_used;
                                num_samples=num_samples_adaptive_lp, P_occ=P_occ, rng=rng)
                        orig_lp_adaptive, _, orig_lp_adaptive_failed_solves,
                            orig_lp_adaptive_total_solves =
                            simulate_policy_original_lp_adaptive(jobs, weights_use, m_used;
                                num_samples=num_samples_adaptive_lp,
                                num_scenario_samples=num_scenario_samples, rng=rng)
                    end
                    ...
                    row = Dict(
                        ...
                        :lp_cons_status=>lp_cons_status, :lp_orig_status=>lp_orig_status,
                        :cdsrse_weight=>cons_w, :cdsrse_ratio=>cons_r,
                        :dsrse_weight=>orig_w, :dsrse_ratio=>orig_r,
                        :cdsrse_lp_adaptive=>cons_lp_adaptive,
                        :dsrse_lp_adaptive=>orig_lp_adaptive,
                        :cdsrse_lp_adaptive_failed_solves=>cons_lp_adaptive_failed_solves,
                        :cdsrse_lp_adaptive_total_solves=>cons_lp_adaptive_total_solves,
                        :dsrse_lp_adaptive_failed_solves=>orig_lp_adaptive_failed_solves,
                        :dsrse_lp_adaptive_total_solves=>orig_lp_adaptive_total_solves
                    )
                    push!(rows, row)
                    if checkpoint_raw_path !== nothing
                        df_checkpoint = DataFrame(rows)
                        add_ratio_columns!(df_checkpoint)
                        add_gap_columns!(df_checkpoint)
                        CSV.write(output_path(checkpoint_raw_path), df_checkpoint)
                    end
                end
            end
        end
    ...
    CSV.write(output_path("final_experiment_new_raw.csv"), df_raw)
    ...
    CSV.write(output_path("final_experiment_new.csv"), df_avg)
    ...
    for dt in unique(df_avg.dist_type), dens in unique(df_avg.density), len in unique(df_avg.length)
        ...
        if "dsrse_lp_adaptive_over_opt" in names(sub)
            plot!(p_dsrse, xticks, sub.dsrse_lp_adaptive_over_opt; lw=2,
                marker=:hexagon, label="DSRSE adaptive LP")
        end
        savefig(p_dsrse, output_path("plot_dsrse_$(dens)_$(len)_$(dt).png"))
        display(p_dsrse)
        ...
        if "cdsrse_lp_adaptive_over_opt" in names(sub)
            plot!(p_cdsrse, xticks, sub.cdsrse_lp_adaptive_over_opt; lw=2,
                marker=:hexagon, label="CDSRSE adaptive LP")
        end
        savefig(p_cdsrse, output_path("plot_cdsrse_$(dens)_$(len)_$(dt).png"))
        display(p_cdsrse)
    end
