...
%% =========================================================
%% USER PACKAGES AND MACROS FROM ORIGINAL MANUSCRIPT
%% =========================================================
...
\usepackage{mdframed}
\usepackage{mathtools,flexisym,bbm}
\usepackage{xcolor,paralist,fancyhdr,etoolbox}
\usepackage{mathrsfs}
\usepackage{pgfplots}
\usepackage[shortlabels]{enumitem}
\usepackage{algorithm}
\usepackage[noend]{algpseudocode}
% \usepackage{cite} % elsarticle handles citations internally usually, but natbib is loaded by default.
\usepackage{multirow}
...
% \usepackage[margin=1in]{geometry} % Commented out to let elsarticle handle margins
...
\title{Dynamic Interval Scheduling with Random Start and End Times\tnoteref{t1}}
\tnotetext[t1]{Partially funded by the U.S. Office of Naval Research, N00014-23-1-2631.}
...
\author[gatech]{Rui Gong\corref{cor1}}
\ead{rgong44@gatech.edu}
\cortext[cor1]{Corresponding author}

\author[gatech]{Alejandro Toriello}

\affiliation[gatech]{organization={Georgia Institute of Technology},
            city={Atlanta},
            state={GA},
            postcode={30332}, 
            country={USA}}
...
\begin{keyword}
Interval scheduling \sep Stochastic optimization \sep Dynamic programming \sep Linear programming relaxation
...
& \sum_{t}\biggl(\sum_{i, a_i\leq k\leq d_i} x_i^t 
  - \sum_{i,a_i\leq k\leq b_i}\sum_{\ell = k+1}^{b_i}x_{i\ell}^{ts} \nonumber \\
& \qquad\quad - \sum_{i,c_i\leq k\leq d_i}\sum_{\ell =c_i}^{k-1}x_{i\ell}^{te}\biggr) \leq 1,
  ~\forall k\in M \\
    &\sum_{t}x_{ik}^{ts}\leq P_{ik}^{s},~\sum_{t}x_{ik}^{te}\leq P_{ik}^{e},\forall i\in N,\forall k\in M\\
    \end{split}
\end{align*}
...
Thus,
\begin{align*}
\sum_r [\mu_{\pes}]_r\leq & \alpha^*\\
\leq &\sum_r [\mu_{\pes}]_r\left(1+\sum_{i\text{ s.t. }r\in S_i} (1-P_{ir}^o)+\sum_{i\text{ s.t. }r\in E_i} (1-P_{ir}^o)\right).
\end{align*}
...
\begin{align*}
   ...
   =& 1+\sum_{i,a_i\leq k\leq b_i}\sum_{\ell = k+1}^{b_i}x_{i\ell}^{s}+\sum_{i,c_i\leq k\leq d_i}\sum_{\ell =c_i}^{k-1}x_{i\ell}^{e}\\
   &+\sum_{i, P_{ik}^o = 0} x_i.
\end{align*}
Let $k_i$ be the unique element in $\mathcal{J}$ covering $i\in[n]$.
Summing both sides over $\mathcal{J}$, we get
\begin{align*}
    \alpha*\alpha_{\pes}\leq  &\alpha_{\pes}+\sum_{k\in\mathcal{J}}\left(\sum_{i,a_i\leq k\leq b_i}\sum_{\ell = k+1}^{b_i}x_{i\ell}^{s}+\sum_{i,c_i\leq k\leq d_i}\sum_{\ell =c_i}^{k-1}x_{i\ell}^{e}\right)\\
    &+(\alpha_{\pes}-1)\sum_{i}x_i\\
    \iff
