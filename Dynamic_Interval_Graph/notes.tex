...
\end{abstract} %%%%%%%%%
\maketitle
\section{Dynamic Interval Graphs}
Consider a finite set of tasks whose ending time is known and the start time following an independent uniform distribution $U_i$ for each task $i$ (it is the same if we assume the starting time is known but the ending time is not); each task has a weight $w_i>0$ and we want to dynamically select a stable set maximized the expected weight of the stable set.

Consider the following algorithm, assume tasks to be sorted such that task $i$ has the $i$-th earliest ending time.
Build a value function $\calV(i)$ for ${1,\ldots,i}$ as 
\[\calV(i)=\max(w_i+\E[\calV(j)],\calV(i-1)), \calV(0)=0\]
where $\{1,\ldots,j\}$ is the remaining subset of vertices of $\{1,\ldots,i\}$ after selecting $i$ and realizing its starting time.

Consider the following algorithm:

    \begin{algorithm}
    \begin{algorithmic}
        \State Starting from $N=\{1,\ldots,n\}$.
        \While{$N\neq\emptyset$}
            \State $i=\max\{j:j\in N\}$
            \If{$\calV(i)>\calV(i-1)$}
            \State Select $i$
            \State Update $N$ after realizing the starting time of task $i$
            \Else 
            \State update $N=N\setminus\{i\}$.
            \EndIf
        \EndWhile
...
\begin{lemma}
    The algorithm above is the optimal policy for this problem.
\end{lemma}
\begin{proof}
    Prove this by induction.
    Consider the base case where $|N|=1$, say $N=\{1\}$, since $\E[\calV(j)]=0$ and $\calV(0)=0$, we have 
    $\calV(1)=1+0>0$. So $1$ is selected which is the optimal action for this case. And we see that $\calV(1)$ is the expected value of an optimal policy.

    Suppose that for any $i<n$, the policy is optimal for $|N|=i$, say $N=[i]$, and $\calV(i)=\E[\alpha([i])]$, we show that the policy is also optimal for $[n]$.
    First, if $\calV(n)=\calV(n-1)$, then by definition, the expect optimal value of $[n]$ is equivalent to $[n-1]$, so $n$ can be deleted.
    Otherwise, expected value of selecting $n$ is greater than the expected value of not selecting it ($\calV(n-1)$), so we should select.
    Suppose $j<n$ is selected, then its value would be $w_j+\E[\calV(l)]+\E[\calV(\{j+1,\ldots,n\}\setminus\delta_j]\leq\calV(j)+\calV(\{j+1,\ldots,n\})\leq \calV(n)$.
    Indeed, we know either selecting $n$ or deleting $n$ is the optimal action.
\end{proof}
\begin{lemma}
    This algorithm above is polnomial.
\end{lemma}
\begin{proof}
    Suppose $n$ is selected from $[n]$, then the number of possible states is $n-1$, including sets $\{1\},\{1,2\},\ldots,\{1,\ldots,n-1\}$. So the number of possible states are $O(n(n-1)^2)$ and each state has two actions to consider.
\end{proof}
...
\begin{align}
  ...
  \tag{SDP-D}
  \begin{split}
    \min_{t,Q}\quad &t+\sum_{ij\in{n\choose{2}}}Q_{ij}(1-p_{ij})\\
    \text{s.t.}\quad &
    Q_{ii}=-2q_i-w_i\\
    % &Q_{ij}\leq \lambda_{ij}\\
    &Q_{ij}\geq 0\\
    &
    \begin{pmatrix}
      t&q^\top\\
...
\subsection{Dynamically Selecting Stable Sets Given Embedding Properties}
As above, consider a random graph $G$ with vertex set $V$ and a random edge set $E$, and a weight function $w$.
We consider the embedding of the random graph. 
For example, we say that $G$ is embedded by perfect graphs if 
$\Pr[G=T] = 0$ for every $T=(V,\tilde{E})$ being not perfect; in other words, any realization of this random graph $G$ is perfect.

For every pair $(i,j)\in E$ with $i\neq j$, we defined the probability 
\[p_{ij}=\Pr[ij\text{ is realized if it can be either $1,0$ without violating the embedding.}]\]
Consider the following way of realizing a $G$:
\begin{enumerate}
    \item Let $\tilde{E}\subseteq \binom{n}{2}$ be the set of realized edges.
    \item If there exists $ij\in\binom{n}{2}$ such that $ij$ being $1$($0$) violates the embedding structure, then let $ij$ be $0$($1$).
    Add $ij$ to $\tilde{E}$.
    \item Uniformly pick a remaining edge and realized it with $\operatorname{Ber}(p_{ij})$.
    \item Repeat $2$ and $3$ until no edge remains undetermined.
\end{enumerate}
Certain embeddings can be realized by realizing edges one by one.
For example, perfect graphs.
\begin{example}
    Consider a random graph $G$ with $V=[6]$ whose realized edges are $12,23,16$;
    undetermined edges are $34,45,35,56$ and all remaining edges are not realized.
    
    Let $G$ be embedded by perfect graphs. Thenb, at this stage, any remaining undetermined edge can either be $0$ or $1$.
    Suppose that then the edge $56$ is realized, we know if $35$ is realized, then $12356$ forms an induced odd cycle, which violates the perfectness of the embedding.
    Thus, $35$ is determined to be not realized.
    Then, $34,45$ can either be $0$ or $1$.
\end{example}

There are some graph structures that the above model does not apply to.
That is, realizing some edges will affect the probability $p_{ij}$.
\begin{example}
    Consider $G$ with vertex set $[5]$ whose embedding is the interval graphs of $[5]$ where the starting times $t_1\leq t_2\leq\ldots\leq t_5$.
    Suppose the realized edges are $13,25$, then by the property of embedding graphs,
     we know $24,12,23$ has to be realized.
     But for $34,35,45$, each of them can be either $0$ or $1$.
     However, suppose $34,45$ are realized. 
     Still, $35$ can be $0$ or $1$, but the probability can change.
     Say $3$ starts at $0$, $4$ at $1$, $5$ at $1.5$, and the end time of $3$ is uniformly distributed on $[0,2]$.
     Then without any given information, the probability of $35$ exists is $1/4$.
     Given $34$ is realized, the probability of $35$ realized become $1/2$.
     However, if we consider another model where for each task, instead of realizing it overlapping with other tasks, we realize its end time.
     Then all tasks whose starting time is before the end time are connected with the task and all others are not.
\end{example}
The problem we want to consider is, for both such models, we know we realized a perfect graph or a interval grpah (which is indeed perfect and chordal) in the end, how can we find the expected stability number or dynamically select vertex to the stable set.

\begin{proposition}
    Let $G$ with vertex set $N$, $p_{ij}$ for each $ij\in \binom{n}{2}$ be as above and being embedded with perfect graphs.
    Let $G'$ be a subgraph with vertex set $V\subseteq N$ being an arbitrary subset of $N$ and no edge realized.
    Then the probability of $ij$ appear in $G'$ is also $p_{ij}$.
\end{proposition}
\begin{proof}
    By the definition of the model, since $G'$ is a graph with no edges realized, realizing $p_{ij}$ does not violate the perfectness, thus the realization of $ij$ follows an independent Bernoulli distribution.
\end{proof}

With the above settings, consider the following two models. 
For a random graph $G$ with vertex set $N$ and embedded with perfect graphs, first determine all edges with probability $1$ or $0$ to appear.
Then determine all edges necessary to the embedding.
When a vertex selectable $i$ is selected to the stable set, realize its neighbors with independent $\operatorname{Bernoulli}(p_{ij})$ for each $j$ with $p_{ij}\in (0,1)$, mark $j$ where $ij$ realized as not selectable.
Determine all edges necessary to the embedding. Keep repeating.
\begin{lemma}
    With the above process until the graph is empty, let $(u_i,\delta_i)$ be the $i$-th selected vertex and its realized neighbors for $i=1,\ldots,t$.
    Then $G|_{\cup_{i=1}^{t}\{u_i,\delta_i\}}$ is a perfect graph.
\end{lemma}
\begin{proof}
    By the definition of the process.
\end{proof}
For the random interval graph with given starting point, the process is similar.
For each task, once we decide to select it, we realize its finish time and mark anything tasks starting with this interval and any tasks that is possible to finish within this interval as deleted.
Such interval graphs can be described by directed graphs.
...
\begin{align*}
  ...
  &X_{(i,t),(j,\tau)}\leq (1-p_{ij})x_i^t~\max_{H\subseteq H\setminus\{i,j\},|H|=\tau-t-1}\prod_{k\in H}(1-p_{ik})(1-p_{jk}),\quad\forall i,j,t<\tau\\
